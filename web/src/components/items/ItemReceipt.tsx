...
import { organisationHref, recordHref } from "@/lib/links";

type ItemDetail = components["schemas"]["ItemDetail"];
...
// A record printed as one long receipt. Shared by the full site's record page and the static
// edition's, so both show the same thing.
export function ItemReceipt({
  item,
  similar,
  qrUrl,
  calendar,
  ...
  item: ItemDetail;
  similar: { cpv: string; items: ItemSummary[] } | null;
  qrUrl: string;
  /** The .ics download needs the full site's server. */
  calendar: boolean;
}) {
        ...
        <div className="relative z-10 -mb-9 mr-4 self-end rotate-2 sm:mr-10">
          <div className="slip-shadow">
            <p className="slip flex flex-col gap-1 pt-3 pr-4 pl-9 font-mono text-xs">
              ...
              <Link href={recordHref(item.supersededBy)} className="text-sm underline">
                {item.supersededBy}
              </Link>
            </p>
          </div>
      ...
      <div className="slip-shadow relative">
        <article className={`slip flex flex-col gap-5 px-5 sm:px-10 ${item.supersededBy ? "pt-14 sm:pt-12" : "pt-6 sm:pt-9"}`}>
          <ReceiptHead
            ...
            name={
              item.organisation && (
                <Link href={organisationHref(item.organisation.id)} className="no-underline hover:underline">
              ...
              <ul className="flex flex-col gap-2.5">
                {similar.items.map((other) => (
                  <li key={other.sourceId} className="flex flex-col gap-0.5">
                    <Link href={recordHref(other.sourceId)} className="leading-snug underline">
                      {other.title}
                    </Link>
